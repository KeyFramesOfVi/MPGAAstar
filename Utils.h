...
#ifndef MPGAAstar_Utils_h
#define MPGAAstar_Utils_h
...
public:
    ...
    // uses gsl libraries function to sample from a gaussian distribution (also uses a version of box_muller transform)
    static double getGaussianSample(const gsl_rng* r, double mean, double variance) {
        return mean + gsl_ran_gaussian( r, sqrt(variance) );
    }
